...
import com.jichuangsi.school.teacher.R;
...
public class FreeDrawView extends View implements View.OnTouchListener {
    ...
    private void initPaints(TypedArray a) {
        ...
        mCurrentPaint.setStrokeWidth(a != null ? a.getDimensionPixelSize(R.styleable.FreeDrawView_paintWidth, (int)
                FreeDrawHelper.convertDpToPixels(DEFAULT_STROKE_WIDTH)) : FreeDrawHelper.convertDpToPixels
        ...
        if (a != null) {
            ...
            mResizeBehaviour = resizeBehaviour == 0 ? ResizeBehaviour.CLEAR : resizeBehaviour == 1 ? ResizeBehaviour
                    .FIT_XY : resizeBehaviour == 2 ? ResizeBehaviour.CROP : ResizeBehaviour.CROP;
        }
    }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        for (HistoryPath currentPath : mPaths) {
            ...
            if (currentPath.isPoint()) {
                canvas.drawCircle(currentPath.getOriginX(), currentPath.getOriginY(), currentPath.getPaint()
                        .getStrokeWidth() / 2, currentPath.getPaint());
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        if (mMode == Mode.ERASER) {
            ...
            if ((motionEvent.getAction() != MotionEvent.ACTION_UP) && (motionEvent.getAction() != MotionEvent
                    .ACTION_CANCEL)) {
                Point point;
    ...
    public void setDataComming(boolean isDrawing) {
        mFinishPath = !isDrawing;
        if (mFinishPath) {
            invalidate();
        }
    }
