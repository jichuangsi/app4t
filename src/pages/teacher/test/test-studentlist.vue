  ...
  export default {
    ...
    data() {
      return {
          ...
          jump: {name: '开始批改', url: '/testquestions'},//添加主观题按钮
          header: {                                        //头部标题
            url: '/testIndex',
            title: ''
          }
      ...
      computed: {
          ...
          filtersTextChange() {
              ...
              let arr = [];
              if (_this.inputValue === '') {
                  return _this.testStudents;
              } else {
                  for (let i = 0; i < _this.testStudents.length; i++) {
                      if(_this.testStudents[i].studentName.indexOf(_this.inputValue) > -1 ||
                          _this.testStudents[i].studentAccount.indexOf(_this.inputValue) > -1){
                          arr.push(_this.testStudents[i]);
                      }
                  }
                  return arr;
              }
