    ...
    <div class="objectiveWork">
        ...
        <div class="objective_content">
            ...
            <div class="student_msg">
                <div class="msg">
                    <div class="info" v-html="homeworkInfo"></div>
                    <!--<div class="class"><span>班级</span>&nbsp;初一（1）班</div>-->
                </div>
                <div class="name"><span>学生姓名：</span><span>{{this.studentName}}</span><span v-show="!homeworkCompleted">，未完成。</span><span v-show="homeworkCompleted">，已完成。</span></div>
                ...
                <div class="clearfix">
                    <div class="right submit" @click.stop.passive="next"></div>
                </div>
            </div>
            <div class="topic_warp">
                <div class="objective_warp" v-for="(item,index) in this.homeworkObjectiveQs" :key="index">
                    <common-objective :objective="item" :index="index" :completed="homeworkCompleted"/>
                </div>
            </div>
        </div>
...
<script>
    ...
    import commonObjective from "../../../../components/topicList/commonObjective"
    import {mapGetters} from 'vuex'
    import store from '@/store'
    import {getStudentHomework} from "@/api/teacher/homework"
    ...
    export default {
        ...
        data() {
            return {
                ...
                loading: true,                 //加载状态
                pageShow: false,               //页面状态
                jump: {name: '转至主观题', url: '/TStudentSubjective'},              //头部多加一个按钮
                objectiveAnswer: [],
                homeworkInfo: '',
                objectiveTotal: 0,
                objectiveCorrect: 0,
        ...
        mounted() {
            this.studentId = this.$route.query.studentId?this.$route.query.studentId:this.homeworkStudentId;
            this.studentName = this.$route.query.studentName?this.$route.query.studentName:this.homeworkStudentName;
            if(this.$route.query.studentId) store.commit('SET_HOMEWORKSTUDENTID', this.studentId);
            if(this.$route.query.studentName) store.commit('SET_HOMEWORKSTUDENTNAME', this.studentName);
            this.getStudentHomeworkDetail();
        },
        methods: {
            getStudentHomeworkDetail(){
                getStudentHomework(this.homeworkId, this.studentId).then(res=>{
                    ...
                    res.data.data.questions.forEach((q, index) => {
                        switch (q.questionType) {
                            case 'objective' : {
                                objectiveQs.push(q);
                                if(res.data.data.completed&&q.answerModelForStudent
                                ...
                                }else if(!res.data.data.completed&&q.answerModelForStudent
                                    &&q.answerModelForStudent.answerForObjective){
                                    count++;
                            ...
                            case 'subjective' : {
                                subjectiveQs.push(q);
                                if(res.data.data.completed&&q.answerModelForTeacher
                    ...
                    store.commit('SET_HOMEWORKCOMPLETED', res.data.data.completed);
            ...
            next(){
                ...
                let i = this.homeworkStudents.findIndex(x=>{
                   return x.studentId ===  this.studentId;
                });
                if(i != -1){
                    if(i === this.homeworkStudents.length - 1){
                        this.studentId = this.homeworkStudents[0].studentId;
                        this.studentName = this.homeworkStudents[0].studentName;
                        store.commit('SET_HOMEWORKSTUDENTID', this.studentId);
                        store.commit('SET_HOMEWORKSTUDENTNAME', this.studentName);
                        this.getStudentHomeworkDetail();
                    }else{
                        if(this.homeworkStudents[i+1].completedTime>0){
                            this.studentId = this.homeworkStudents[i+1].studentId;
                            this.studentName = this.homeworkStudents[i+1].studentName;
                            store.commit('SET_HOMEWORKSTUDENTID', this.studentId);
                            store.commit('SET_HOMEWORKSTUDENTNAME', this.studentName);
                            this.getStudentHomeworkDetail();
                        }else{
        ...
        .objective_content {
            ...
            .student_msg {
                ...
                .submit {
                    ...
                    background-position: -135px -3023px;
                    // background-color: #69B482;
                }
                .submit:active {
                    background: url('../../../../assets/按钮.png') no-repeat;
                    background-position: -592px -3023px;
